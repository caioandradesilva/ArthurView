...
import type { Site, Container } from '../../types';
...
const CreateRackModal: React.FC<CreateRackModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  selectedContainerId
  ...
  useEffect(() => {
    if (selectedContainerId) {
      setFormData(prev => ({ ...prev, containerId: selectedContainerId }));
    }
  }, [selectedContainerId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await FirestoreService.createRack({
        name: formData.name,
      ...
      onSuccess();
      setFormData({ name: '', containerId: selectedContainerId || '', siteId: '', capacity: 24 });
    } catch (err: any) {
      setError(err.message || 'Error creating rack');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Add New Rack</h2>
          <button
            onClick={onClose}
